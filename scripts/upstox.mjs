...
export async function ivFromOptionQuotes(token, options, F, expiryIso) {
  try {
    ...
    // Wide window: covers far-OTM strikes sellers actually use (position P&L
    // reads the live quote for the exact strike when it's in this set).
    const wanted = new Set(strikes.slice(Math.max(0, idx - 25), idx + 26));
