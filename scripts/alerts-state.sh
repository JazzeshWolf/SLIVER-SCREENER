...
# have already been announced (metals.json).
#
...
#
# NEVER force-push this branch. It is the only memory of what was already sent:
# a force push that loses a commit re-announces everything that commit tracked
# (or, worse, forgets a DROPPED it never delivered). Ordinary commits of one
# small JSON file rebase cleanly, and the history doubles as an audit log of
# what was sent when. Deleting the branch is the supported way to reset — the
# next run re-arms with ONE summary message.
set -euo pipefail
...
case "${1:-}" in
  ...
  push)
    cd "$DIR"
    git config user.name "sliver-bot"
    ...
    for attempt in 1 2 3 4 5; do
      ...
      echo "push rejected (attempt $attempt) — rebasing onto the newer alerts-state."
